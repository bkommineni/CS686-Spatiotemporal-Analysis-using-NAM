...
public class SolarAndWindFarmReducer extends Reducer<Text,Text,Text,Text> {
    ...
    protected void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
        ...
        for(Text t: values)
        {
            ...
            float cloud_cover = Float.parseFloat(tokens[1]);
            sum_cloud_cover = sum_cloud_cover + cloud_cover;
        ...
        context.write(key, new Text(Double.toString(sum_wind_speed/count) +","
                            +Float.toString(sum_cloud_cover/count)+","+Float.toString(sum_temperature/count)));
    }
}
